    ...
    axios
      ...
      .catch((err) => {
        if (err.response && err.response.status === 400) {
          setInvalidInput(true);
          setErrorDetails(`User-ID "${values.email}" is already taken!`);
          return;
        } else {
          setInvalidInput(true);
          setErrorDetails(
            `Please verify if you have your node server running on port "7050" and you have implemented all the mentioned APIs! If you need help try accessing the hints!`
          );
        }
      });
  };
